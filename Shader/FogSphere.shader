Shader "InPlanaria/SimpleVolumetric/FogSphere"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                // 8. エッジフェード（Fresnel効果）
                // 球の表面法線を計算
                float3 surfaceNormal = normalize((ro + rd * t2));
                float fresnel = abs(dot(-rd, surfaceNormal));
                fresnel = smoothstep(0.0, UNITY_ACCESS_INSTANCED_PROP(Props, _EdgeFade), fresnel);
